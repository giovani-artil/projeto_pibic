...
if __name__ == "__main__":
	...
	PATH = '/app/client_foco_aleatorio/Viewport/ds{}/'.format(dataset)
		...
		for vector in vector_ds:
			...
			for f in np.arange(0, max_frame):
				...
				if(len(series_t) != 0):
					...
					base = np.mean(np.array(series_v), axis=0)
					noise = np.random.normal(scale=0.75, size=3)
					mean_series_v = base + noise
					mean_series_v = mean_series_v / np.linalg.norm(mean_series_v)
